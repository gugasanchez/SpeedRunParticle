# Challenge #0: SpeedRun Particle Auth
...
1. Checkpoint 0: 📦 Setting Up the Environment 📚
...
After creating the project, go to the project settings to find the Project ID, Client Key, and App ID (for the App ID, you can use a mock Url like _com.company.name_ if your application still doesn't have one).
...
Change the _defaultNetwork_ in `packages/hardhat/hardhat.config.ts` to Sepolia.
